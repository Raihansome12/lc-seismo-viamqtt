...
class ADS1256:
    ...
    def ADS1256_Read_ADC_Data(self):
        ...
        config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        #Gabungkan 3 byte menjadi satu nilai 24-bit
        read = (buf[0] << 16) | (buf[1] << 8) | buf[2]
    
        # Handle nilai negatif (two's complement)
        if read & 0x800000:  # Jika bit paling signifikan (MSB) adalah 1 (nilai negatif)
            read = read - 0x1000000  # Konversi ke nilai negatif
