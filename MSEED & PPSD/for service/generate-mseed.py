...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, 'mseed-generator.log')
    ...
    # Setup logger
    logger = logging.getLogger('mseed-generator')
...
def detect_sessions(parsed_data):
    ...
    EXPECTED_DIFF = timedelta(seconds=1.0)
    MAX_DIFF = timedelta(seconds=3.0)
...
def service_main_loop():
    ...
    while running:
        try:
            ...
            if not conn:
                ...
                sleep_stepwise(SERVICE_CONFIG['retry_delay'])
                continue
            ...
            # Wait before next check (dengan sleep bertahap)
            if running:
                logger.info(f"Waiting {SERVICE_CONFIG['check_interval']} seconds before next check...")
                sleep_stepwise(SERVICE_CONFIG['check_interval'])
    ...
    logger.info("Seismic data processor service stopped cleanly")

def sleep_stepwise(total_seconds):
    """Melakukan sleep secara bertahap agar bisa di-interrupt oleh SIGTERM"""
