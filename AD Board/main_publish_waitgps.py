...
class GeophonesensorADC:
    ...
    def run(self):
        self.running = True
        print("Geophone: Waiting for GPS to get satellite fix...")
        ...
        if program_stop_event.is_set():
            print("Geophone: Program stopped before GPS ready")
            return
            
        print("Geophone: GPS ready, starting data collection...")
        sample_counter = 0
        ...
        try:
            while self.running and not program_stop_event.is_set():
                ...
                if sleep_time > 0:
                    ...
                    print(f"WARNING: Geophone sampling taking too long! ({elapsed:.4f}s > 0.025s)")
...
class GPSSensor:
    ...
    def check_gps_fix(self):
        ...
        while attempts < max_attempts:
            try:
                ...
                print(f"Error checking GPS fix: {e}")
                self.initialize()
                attempts += 1
    ...
    def wait_for_gps_fix(self):
        ...
        while not program_stop_event.is_set():
            if self.check_gps_fix():
                print("GPS: Satellite fix acquired!")
                self.gps_fix_status = True
                gps_ready_event.set()  # Beritahu thread lain bahwa GPS sudah siap
                return True
            else:
                print("GPS: No satellite fix yet, retrying in 10 seconds...")
    ...
    def run(self):
        ...
        if not self.wait_for_gps_fix():
            print("GPS: Program stopped before getting satellite fix")
            return
        
        print("GPS: Starting data collection...")
        ...
        try:
            while self.running and not program_stop_event.is_set():
                ...
                if data:
                    ...
                    print("GPS: Lost satellite fix, checking status...")
                    # Jika kehilangan fix, cek status dan tunggu lagi jika perlu
                    if not self.check_gps_fix():
                        print("GPS: Satellite fix lost! Stopping all data transmission...")
...
def main():
    ...
    try:
        ...
        print("Note: Geophone data will only be sent after GPS gets satellite fix")
        ...
        program_stop_event.set()  # Set event untuk menghentikan semua thread
        geophone.stop()
        gps.stop()
