...
MSEED_DIR = "/home/skripsi/seismic-instrument/mseed_files"  # Direktori untuk menyimpan file miniSEED lokal (backup)
...
class DataLogger:
    ...
    def test_api_connection(self):
        ...
        url = "http://203.81.248.138:2001/api/ping"  # Endpoint ping sederhana di Laravel
    ...
    def generate_mseed_file(self):
        ...
        try:
            ...
            trace = Trace(data=adc_values)
            
            # Isi metadata ke trace.stats
            ...
            trace.stats.starttime = UTCDateTime(self.start_time)
            trace.stats.sampling_rate = 50.0  # 50 SPS
            trace.stats.npts = len(adc_values)
    ...
    def upload_mseed_file(self, filename, mseed_binary):
        """Kirim file miniSEED ke Laravel API untuk disimpan di database"""
        if not filename or not mseed_binary or not self.start_time or not self.end_time:
            print("No data to upload to API")
            return False

        url = "http://203.81.248.138:2001/api/upload-mseed"

        try:
            files = {
                'file': (filename, mseed_binary, 'application/octet-stream')
            }
            data = {
                'filename': filename,
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(f"Response is not valid JSON or empty. Raw response: {response.txt}")
                return True
            else:
    ...
    def finish_session(self):
        ...
        if filename and mseed_binary:
            # Simpan ke database
            success = self.upload_mseed_file(filename, mseed_binary)
            
            if success:
                print("=== Session completed successfully ===")
            else:
                print("=== Session completed but upload mseed failed ===")
        else:
...
def setup_mqtt():
    def on_connect(client, userdata, flags, reasonCode, properties):
        if reasonCode == 0:
            ...
            print(f"Failed to connect to MQTT broker with error code: {reasonCode}")
    
    def on_disconnect(client, userdata, reasonCode, properties, sock):
        print(f"Disconnected from MQTT broker with error code: {reasonCode}")
